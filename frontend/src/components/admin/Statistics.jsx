  ...
  return (
    <div className="space-y-6">
          ...
          <div className="space-y-4">
            <div>
              <div className="flex justify-between text-xs font-bold mb-1.5">
                ...
                <span className="text-slate-900">{summary.totalStudents} ({getPercentage(summary.totalStudents, summary.totalStudents + summary.totalMentors)})</span>
              </div>
            ...
            <div>
              <div className="flex justify-between text-xs font-bold mb-1.5">
                ...
                <span className="text-slate-900">{summary.totalMentors} ({getPercentage(summary.totalMentors, summary.totalStudents + summary.totalMentors)})</span>
              </div>
              <div className="w-full bg-slate-100 rounded-full h-3">
                <div 
          ...
          <div className="space-y-3">
            {charts.popularSubjects.length === 0 ? (
              ...
              charts.popularSubjects.map((sub, idx) => {
                const colors = ['bg-brand-600', 'bg-indigo-600', 'bg-violet-600', 'bg-purple-600', 'bg-pink-600'];
                return (
                  <div key={sub.subject}>
                    <div className="flex justify-between text-xs mb-1">
                      <span className="text-slate-700 font-bold">{sub.subject}</span>
                      <span className="text-slate-500 font-medium">{sub.count} bookings</span>
                    </div>
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm text-left">
        ...
        <div className="flex items-end justify-between h-40 gap-4 pt-4 border-b border-slate-100 px-4">
          {charts.sessionsOverTime.length === 0 ? (
            <div className="w-full text-center text-xs text-slate-400 italic pb-16">
              No historical data available.
            </div>
          ) : (
            charts.sessionsOverTime.map((pt) => (
              <div key={pt.date} className="flex-1 flex flex-col items-center group">
                <span className="text-[10px] text-brand-600 font-bold opacity-0 group-hover:opacity-100 transition-opacity mb-1">
                  {pt.count}
                </span>
                <div 
                  className="w-full max-w-[28px] bg-gradient-to-t from-brand-600 to-indigo-500 rounded-t-lg transition-all duration-300 group-hover:brightness-110"
                  style={{ height: `${Math.max(pt.count * 15, 12)}px` }}
                />
                <span className="text-[10px] text-slate-400 font-medium mt-2">{pt.date}</span>
              </div>
            ))
          )}
        </div>
